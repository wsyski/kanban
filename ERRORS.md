...
### O8. An IDE changelist commit swept the tree mid-run — and deleted three documents

Two commits landed at 21:25:01 and 21:25:28 on 2026-09-11, both from the
operator's IDE (`.idea/workspace.xml`: changelist "Generic kanban", commit
comment "Generic kanban plan"), i.e. from outside any session, while run 5 was
mid-lane:

- the first swept the whole changeset — in-flight edits to `run.py`, the tests,
  `ERRORS.md` and this plan, plus the two `runs/artifacts/lane-1/*.md` files the
  live run had force-staged. Nothing generated is ever committed, so the
  guard in `test_shipped_boards.py` went red against the repo's own HEAD;
- the second deleted `ERRORS.md`, the plan and the spec — 3284 lines, no
  renames, nothing moved anywhere — and pushed the deletion to `origin/main`.

Both were repaired by restoring from the last good commit (`0c151d7`, which had
the newest copies) and untracking the generated paths; the files never left git.

Rules: a commit made while a driver is live is suspect — check `git log --stat`
for `boards/*/work|runs` additions and for missing documents. Generated paths
that reach HEAD get untracked (`git rm --cached`, the file stays on disk and the
next run resets it) — the guard test only tolerates them as a *pending* removal.
A changelist-wide commit has no pathspec, so it will always include whatever a
run has staged at that moment.
