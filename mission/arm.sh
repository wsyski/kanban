...
#
# Usage: mission/arm.sh <slug> [lane]        (lane defaults to 1)
#
# The board must be SERVING (mission/start-board.sh --slug <slug>), or the card just
...
LANE=${2:-1}
if [ -z "$SLUG" ]; then
  echo "usage: mission/arm.sh <slug> [lane]" >&2
  exit 2
fi
