"""The retired roles, and the model a board may pin its reviews to.

Two rules, one lookup each — `lanes.assignee_for` (which profile works a role) and
`lanes.model_args` (which cards carry the model pin) — so the tests here are about
the rules and about the three places that consume them: the filing path, the rework
path, and the schema that declares the option.
"""
...
def test_the_tester_and_reviewer_roles_run_on_the_coder_profile():
    """Their profiles were retired; their CARDS were not. Every role the graph
    fills has to name a profile that exists, and these two are the coder's — the
    dispatcher files a card whose assignee is not a profile as unspawnable."""
    assert lanes.assignee_for("tester") == "coder"
    assert lanes.assignee_for("reviewer") == "coder"


def test_a_manifest_remap_still_beats_the_fallback():
    assert lanes.assignee_for("reviewer", {"reviewer": "senior"}) == "senior"
    assert lanes.assignee_for("tester", {"tester": "senior"}) == "senior"


def test_a_role_with_a_profile_of_its_own_is_untouched():
    assert lanes.assignee_for("coder") == "coder"
    assert lanes.assignee_for("researcher") == "researcher"
    assert lanes.assignee_for("manager") == "manager"
...
def test_required_profiles_is_the_profiles_that_spawn():
    ...
    req = lanes.required_profiles()
    assert req == sorted(set(req)), req
    assert "human-gate" not in req
    assert {"coder", "researcher", "manager"} <= set(req)
    # the retired roles resolve into the coder, not into profiles of their own
    assert "tester" not in req and "reviewer" not in req


def test_required_profiles_follows_a_remap():
    req = lanes.required_profiles({"reviewer": "senior", "human-gate": "gatekeeper"})
    assert "senior" in req and "reviewer" not in req
...
def test_the_review_cards_take_the_model_the_manifest_pins():
    assert lanes.model_args("reviewer", dict(PINNED)) == [
        "--model", "glm-5.3-flash", "--provider", "opencode-go"]


def test_no_worker_card_carries_the_judges_model():
    """The pin is a property of the JUDGEMENT, not of the board: a coder, a tester
    or a gate files without it and keeps its profile's default model."""
    for role in sorted({r[2] for r in lanes.LANE_CARDS} - lanes.JUDGE_ROLES):
        assert lanes.model_args(role, dict(PINNED)) == [], role


def test_a_board_without_a_model_files_no_model_flag():
    assert lanes.model_args("reviewer", {}) == []
    assert lanes.model_args("reviewer", None) == []
...
def test_a_provider_without_a_model_is_not_sent():
    ...
    assert lanes.model_args("reviewer", {"provider_override": "opencode-go"}) == []


# --- the schema declares both keys ------------------------------------------

def test_the_model_keys_are_board_options_not_per_lane():
    """No header door: the option set decides which keys an idea may carry, and a
    lane must not be able to quietly buy itself a different judge."""
    assert "model_override" in board_schema.BOARD_KEYS
    assert "provider_override" in board_schema.BOARD_KEYS
    assert "model_override" not in board_schema.HEADER_KEYS
    assert "provider_override" not in board_schema.HEADER_KEYS
...
def test_a_re_review_keeps_the_judges_model(monkeypatch, tmp_path):
    """A rework round is reviewed by a review card, so it must carry the same pin:
    without it a second round would silently fall back to the worker's model, which
    is the one thing the pin exists to avoid."""
...
def test_the_pre_flight_accepts_a_board_whose_roles_resolve_to_real_profiles(tmp_path):
    """The retired tester and reviewer roles are the coder's, so a manifest that
    names no assignees needs no profile called tester or reviewer. That is the case
    the hand-written pre-flight list got wrong: it refused EVERY board on a host
    where those two profiles are gone, however the manifest remapped."""
    code, out = _run_create(tmp_path, _board_dir(tmp_path),
                            ["coder", "manager", "researcher"])
...
def test_the_pre_flight_refuses_a_remap_to_a_profile_that_is_not_there(tmp_path):
    ...
    code, out = _run_create(tmp_path, _board_dir(tmp_path, assignees={"reviewer": "senior"}),
                            ["coder", "manager", "researcher"])
