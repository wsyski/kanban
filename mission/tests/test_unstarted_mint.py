...
def _probe_repo(tmp_path):
    """A throwaway repo laid out like this one — `mission/` under the repo root, so
    the script resolves REPO from its own path — holding a copy of create-board.sh
    and symlinks to everything else in mission/, plus one board to file and a
    dispatch lock a live process holds (the pre-flight requires a held one)."""
    ...
    for entry in os.listdir(mission):
        if entry in ("__pycache__", "tests", os.path.basename(CREATE)):
            continue
        os.symlink(os.path.join(mission, entry), repo / "mission" / entry)
    script = repo / "mission" / os.path.basename(CREATE)
    ...
    sample = os.path.join(mission, "..", "boards", "is-even")
