...
## 7. The lane on a checkout with no kanban patch — `goal-smoke`, 2026-09-15

Two source patches in the Hermes checkout existed *because of* this board: the goal judge's
affinity hold (`hermes-kanban-goal-judge-affinity.patch`) and the summary→result mirror
(`hermes-kanban-summary-result-mirror.patch`). Both were retired on 2026-09-15, so
`boards/goal-smoke` was added to measure what dropping them costs — `is-even`'s idea, on
the profile's own (cloud) model, `"goal": true`, `"auto-gates": true`, driven with
`start-board.sh --slug goal-smoke --once` on a checkout reset to `upstream/main`
(`f9ea3a53`) and rebuilt from the four vault assets that remain.

| | |
|---|---|
| run | `runs/goal-smoke-20260915-071024` |
| cards / wall / agent | 9 / 11.3 min / 5.8 min (overhead 5.5, overlap 1.0) of a 4.0 min per-card ceiling |
| per card | I1 1.32, P1 0.87, RVp1 1.62, TW1 0.68 +2, C1 0.95 +2, RVa1 1.03; Gi1/Gp1/Gc1 are gates |
| verdicts | RVp1 PASS, Gp1 PASS, RVa1 PASS, Gc1 PASS |
| audit | 0 errors, 0 warnings (exit 0) |
| doc chain | 0 findings over 9 cards |
| goal judge | `verdict=done` on I1 (07:12:33) and P1 (07:14:07); zero `continue`, zero `MissingSessionID` |
