...
def test_a_board_owned_work_directory_has_no_foreign_index(monkeypatch, tmp_path):
    """Inside this repo the index is shared with the operator's ordinary work in
    `template/` and `driver/`, which is normal — the pathspecs in staged_files() already scope a
    gate's evidence."""
