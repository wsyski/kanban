...
def _env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(run.STATE, "chain_done", set())
...
def _ledger_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(run.STATE, "verdicts_path", str(path))
...
def test_every_placeholder_in_every_shipped_body_is_either_rendered_or_the_workers():
    ...
    import glob
    import file_lanes
    here = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "template")
...
def test_a_restart_that_recorded_nothing_leaves_the_summary_alone(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(run.STATE, "process_recorded", [False])
    run.write_summary({})
...
def test_a_human_gate_records_the_drivers_evidence_beside_the_holders_words(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(run.STATE, "process_recorded", [True])
    monkeypatch.setattr(run.STATE, "gate_evidence", {"Gc1": "12 file(s) staged, verdict PASS"})
...
def test_an_auto_gate_result_is_kept_verbatim(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(run.STATE, "gate_evidence", {"Gc1": evidence})
