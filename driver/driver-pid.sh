...
# repo's driver/run.py counts, however it was started (`driver/run.py`,
# `cd driver; python3 run.py`, an absolute path) — an argument ending in run.py,
# resolved against the process's own cwd. A zombie counts as gone.
alive() { s=$(ps -o stat= -p "$1" 2>/dev/null) && [ -n "$s" ] && [ "${s#Z}" = "$s" ]; }
runs_this_driver() {
  local arg want
  want=$(realpath -m "$REPO/driver/run.py")
