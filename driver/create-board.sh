...
usage() {
cat <<'USAGE'
driver/create-board.sh — create a generic kanban board instance
...
nothing about it lives under template/ or driver/:
...
`python3 template/board_schema.py --schema` prints the option table; this is the
same set:

    {
      "$schema": "../../template/board.schema.json",  # the generated editor schema
...
Every `board.json` carries `"$schema": "../../template/board.schema.json"`, and that
...
under template/ or driver/: the file you edit is the file the board reads, and it stays
...
--slug/--title are required:
    ...
    driver/create-board.sh --slug scratch --title "Scratch"
...
# template/board_schema.py is the one declaration of the option set; it prints
...
if DRIVER_PID=$(live_driver_pid "$BOARD_DIR"); then
  ...
  echo "Stop it first: driver/reset.sh --board ${BOARD_DIR#$REPO/} --batch" >&2
  exit 6
fi
...
import os, sys
sys.path.insert(0, os.path.join(os.getcwd(), "driver"))
